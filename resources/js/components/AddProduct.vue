    ...
    <div class="m-auto w-1/2 mt-10">
        <form @submit="submit" enctype="multipart/form-data">
            <div class="relative z-0 w-full mb-6 group">
                <input type="text" v-model="fields.name" name="name"  class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" required />
            ...
            <div class="relative z-0 w-full mb-6 group">
                <input type="text" v-model="fields.description" name="description" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" required />
            ...
            <div class="relative z-0 w-full mb-6 group">
                <input type="number" min="0" max="1000" v-model="fields.price" onkeydown="return ['Backspace','Delete','ArrowLeft','ArrowRight'].includes(event.code) ? true : !isNaN(Number(event.key)) && event.code!=='Space'" name="price"
            ...
            <div class="relative z-0 w-full mb-6 group">
                <input type="file" v-on:change="onChange" name="photo"  class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" required />
            ...
            <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Añadir Producto</button>
        </form>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        submit(e) {
            ...
            data.append('photo', this.fields.photo);
            axios.post('/store/products', data)
                .then(() => {
                    router.push('home');
                })
                .catch((error) => {
